        ...
        if (userExpandService.checkLimitOpt(MemConstans.SUGGEST_REPLY_LIMIT, sugSuggestReply.getCusId())) {
            ...
            if (falgNum.intValue() == 1) {// 添加SugSuggestReply 成功 建议的回复数加一
                ...
                try {
                	...
                	//发送系统通知
					msgReceiveService.addSystemMessageByCusId(userExpandDto.getShowname()+"回复"+"<a href='"+CommonConstants.contextPath+"/uc/question/info/"+sugSuggestReply.getSuggestId()+"'>"+title+"</a>", sugSuggest.getCusId());
				} catch (Exception e) {
					e.printStackTrace();
				}
