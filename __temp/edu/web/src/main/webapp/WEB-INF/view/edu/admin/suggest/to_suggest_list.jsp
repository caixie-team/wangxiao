...
<body  >
<div class="page_head">
		<h4><em class="icon14 i_01"></em>&nbsp;<span>问题管理</span> &gt; <span>问题列表</span> </h4>
	</div>
<div class="mt20">
	<div class="commonWrap">
	<form action="${ctx}/admin/sug/toSugSuggestList" name="searchForm" id="searchForm" method="post">
		...
		<table cellspacing="0" cellpadding="0" border="0" width="100%" class="commonTab01">
			<caption>
				<div class="capHead">
					...
					<div class="w50pre fl" style="text-align: center;">
						<ul class="ddBar">
							<li>
								<input class="btn btn-danger ml10" type="button" onclick="submitform()" value="查询">
				...
				<div class="mt10 clearfix">
					<p class="fl czBtn"></p>
					<p class="fr c_666"><span>问答列表</span><span class="ml10">共查询到&nbsp;${page.totalResultSize }&nbsp;条记录，当前第&nbsp;${page.currentPage }/${page.totalPageSize }&nbsp;页</span></p>
				</div>
			</caption>
			<thead>
				<tr>
			...
			<tbody  id="tabS_02" align="center">
				...
				<tr id="del${ssl.id }">
					...
					<td>${ssl.title }</td>
					<%-- <td>${ssl.content }</td> --%>
					<td valign="middle" align="center">${ssl.showname }</td>
					<td valign="middle" align="center"><fmt:formatDate value="${ssl.addtime}" pattern="yyyy/MM/dd  HH:mm:ss" ></fmt:formatDate></td>
					<%-- <td valign="middle" align="center">${ssl.hotNum }</td>
					<td valign="middle" align="center">${ssl.wisdomNum }</td>
					<td valign="middle" align="center">
					<c:if test="${ssl.type==1}">务实</c:if>
					<c:if test="${ssl.type==2}">意境</c:if>
					</td> --%>
					<%-- <td id="top${ssl.id }" valign="middle" align="center">
					<c:if test="${ssl.top==1}">是</c:if>
					<c:if test="${ssl.top==0}">否</c:if>
					</td> --%>
					<td valign="middle" align="center">${ssl.replycount }</td>
					<td valign="middle" align="center">${ssl.browseNum }</td>
