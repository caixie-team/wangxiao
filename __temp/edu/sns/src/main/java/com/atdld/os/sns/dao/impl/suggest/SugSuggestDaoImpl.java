package com.atdld.os.sns.dao.impl.suggest;
...
/**
 * @author :
 * @ClassName com.atdld.os.sns.dao.impl.suggest.SugSuggestDaoImpl
 * @description 建议Dao
 * @Create Date : 2013-12-26 下午1:25:52
 */
...
public class SugSuggestDaoImpl extends GenericDaoImpl implements SugSuggestDao {
    ...
    public List<SugSuggest> querySugSuggestListByStatus(SugSuggest sugSuggest, PageEntity page) {
        return this.queryForListPage("SugSuggestMapper.querySugSuggestByType", sugSuggest, page);// 通过type查询建议
    }
    ...
    public List<SugSuggest> queryRecommendSuggestOrderByWisdomNum(QuerySugSuggest querySugSuggest, PageEntity page) {
        return this.queryForListPage("SugSuggestMapper.queryRecommendSuggestOrderByWisdomNumPage", querySugSuggest, page);// 查询推荐的建议的智慧排行分页
    }
    ...
    public Map<String, Object> getSugSuggestCountAfterId(Long SugSuggestId) throws Exception {
        List<QuerySugSuggest> querySugSuggest = this.selectList("SugSuggestMapper.getSugSuggestCountAfterId", SugSuggestId);
        HashMap<String, Object> map = new HashMap<String, Object>();
        map.put(LuceneConstans.LUCENE_LINE_NUM, querySugSuggest.get(0).getLineNum());// 把得到的行数数据放入map中
        map.put(LuceneConstans.LUCENE_MAX_ID, querySugSuggest.get(0).getMaxId());// 把得到的最大建议id数据放入map中
        return map;
    }
