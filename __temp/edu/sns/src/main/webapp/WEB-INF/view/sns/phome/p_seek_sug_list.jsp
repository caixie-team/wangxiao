...
<body class="W-body">
		<!-- 主体 开始-->
								...
								<section class="mt10">
									<header class="comm-title-1">
										...
										<ul class="clearfix">
											...
											<li><a href="${ctx}/p/${userid}/answersug"
												title="回答的问题">回答的问题</a></li>
										</ul>
									</header>
								</section>
								<!-- /挑战信息 -->
										<c:if test="${empty SugSuggestList}">
											<div class="Prompt">
												...
												<p class="vam c-555 fsize14 disIb">您还没有提出过问题呢。</p>
													</c:if>
													<c:if test="${loginId!=userid }">
												<p class="vam c-555 fsize14 disIb">他还没有提出过问题呢。</p>
													</c:if>
											</div>
										</c:if>
											...
											<h5 class="clearfix">
												<div class="fl">
													<a
														...
														class="c-blue article-q-l-link-txt">${sslst.title}</a>
												</div>
											</h5>
											<div class="QA-desc-2">${sslst.shortContent }</div>
											<div class="mt5 clearfix">
												<span class="fr"><tt class="c-888">评论/查看：</tt><a
													...
													class="c-555">${sslst.replycount}&nbsp;/&nbsp;${sslst.browseNum
														}</a></span> <span class="fl ml10"><tt class="c-888">作者：</tt><a
													href="${ctx}/p/${sslst.cusId }/home"
													title="${sslst.showname}" class="c-blue">${sslst.showname}</a></span>
											</div>
										</li>
									</c:forEach>
								</ul>
										...
										<c:forEach items="${sugSuggestHotList}" var="sshlst"
											...
											<dl class="clearfix">
												...
												<dd>
													<div class="cj-c-txt">
														<a class="c-555"
															href="javascript:window.location.href='${ctx}/sug/info/${sshlst.id }'" title="${sswlst.title}">${sshlst.title}</a>
													</div>
												</dd>
											</dl>
										</c:forEach>
				...
				<!-- 主体区域 -->
	<form action="${ctx}/p/${userid}/sug" name="searchForm" id="searchForm" method="post">
