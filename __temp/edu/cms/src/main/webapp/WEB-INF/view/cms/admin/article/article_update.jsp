...
<head>
	...
	$(document).ready(function() {
		initUpdatePageType('articleId',-1,'${stringTypeList}','${article.typeId}','infoTypeId','infoTypeLinkId');
	...
	function addArticleFormSubmit(type){
		...
		if(type==1){
			$("#addArticleForm").attr("action","${ctx}/admin/line/article/updateArticleAndHtml");
		}
...
<body >
<form action="${ctx}/admin/line/article/updateArticle" method="post" id="addArticleForm">
<input type="hidden" name="article.imageUrl" id="imagesUrl" value="${article.imageUrl }"  />
<div class="page_head">
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0" class="commonTab01">
		...
		<tbody>
			<tr>
				...
				<td>
					<input type="text" name="article.title" value="${article.title }" class="{required:true}"/>
				</td>
			</tr>
			<tr>
				<td width="20%" align="center">资讯来源</td>
				<td width="80%">
				<input type="text" name="article.source" value="${article.source }"/>
				</td>
			</tr>
			<tr>
				<td width="20%" align="center">文章摘要</td>
				<td width="80%">
				<textarea rows="5" cols="80" name="article.summary">${article.summary }</textarea>
				</td>
			</tr>
		
			<tr>
				<td align="center">
					<input type="hidden" id="infoTypeId" name="article.typeId" value="${article.typeId}"/>
		    		<input type="hidden" id="infoTypeLinkId" name="article.typeLink" value="${article.typeLink}"/>
			...
			<tr>
				...
				<img src="<%=staticImageServer %>${article.imageUrl}" alt="" id="subjcetpic" width="400px" height="300px"/>
				</c:if>
			...
			<tr>
				...
				<td width="80%">
				<input type="text" name="article.lookNum" value="${article.lookNum }"/>
				</td>
			</tr>
			<tr>
				<td width="20%" align="center">推荐状态</td>
				<td width="80%">
					<input <c:if test="${article.property==0}">checked="checked"</c:if> type="radio" name="article.property" value="0"/>不推荐
	    			<input <c:if test="${article.property==1}">checked="checked"</c:if> type="radio" name="article.property" value="1"/>推荐
	    			<input <c:if test="${article.property==2}">checked="checked"</c:if> type="radio" name="article.property" value="2"/>头条
				</td>
		    </tr>
			<tr>
				<td align="center"><font color="red">*</font>&nbsp;内容</td>
				<td>
					<textarea name="article.content" id="ArticleContent">${article.content }</textarea>
				</td>
			</tr>
			<tr>
				<td align="center" colspan="2">
					<input type="hidden" name="article.articleId" value="${article.articleId }"/>
					<input type="hidden" name="article.linkUrl" value="${article.linkUrl }"/>
					<a class="btn btn-danger" title="保存" href="javascript:addArticleFormSubmit(0)">保存</a>
					<a class="btn btn-danger" title="保存并生成" href="javascript:addArticleFormSubmit(1)">保存并生成</a>
