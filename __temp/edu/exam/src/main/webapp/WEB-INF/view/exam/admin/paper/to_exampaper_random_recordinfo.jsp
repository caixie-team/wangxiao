	...
	<div class="mt20">
		...
		<div class="commonWrap">
			<%--<c:forEach items="${paperMiddleList}" var="divpaperMiddle"
				...
				<div class="mt20 type${divpaperMiddle.qstType}" paperMiddleId="${divpaperMiddle.id}" >
					<div class="testPagerTitle">
					
					<h5>
						<b>第一部分( <c:if
								test="${divpaperMiddle.qstType==1}">单选题</c:if> <c:if
								test="${divpaperMiddle.qstType==2}">多选题</c:if> <c:if
								test="${divpaperMiddle.qstType==3}">判断题</c:if> <c:if
								test="${divpaperMiddle.qstType==4}">材料题</c:if>
								<c:if test="${divpaperMiddle.qstType==5}">不定项选择</c:if>
								<c:if test="${divpaperMiddle.qstType==6}">主观题</c:if> )：
						</b>
						...
						(<span id="title${divpaperMiddle.id }"><c:out value="${divpaperMiddle.qstContent}" /></span>)
						</h5>
						<input type="hidden" value="<c:out value="type"/>" class="${divpaperMiddle.id}type" />
					</div>
					<!-- 循环大题 -->
					<c:choose>
						<c:when test="${divpaperMiddle.qstType==4 }">
						...
						<c:otherwise>
							<div paperMiddleId="${divpaperMiddle.id }" id="paperMiddlediv${divpaperMiddle.id}">
								<div class="testWrap">
									<div>
										&lt;%&ndash; <a class="btn btn-danger" title="随机抽试题" href="javascript:openWindowRandomQuestion('${divpaperMiddle.type }',${divpaperMiddle.id })">随机抽试题</a> &ndash;%&gt;
										<span class="ml10">
            ...
            <c:forEach items="${paperMiddleList}"
                       var="divpaperMiddle" varStatus="index">
            <div >
                <div class="testWrap">
                    <div>

                        <span class="ml10">第${index.count}题</span>
                    </div>
                    <div>
                        <ul class="clearfix mt20 Ul${divpaperMiddle.id }">

                                <li id="li${divpaperMiddle.id }">
                                    <input value="${divpaperMiddle.id }"  type="hidden" />
                                    <div class="ml10">试题id：
                                        <span class="c_333">${divpaperMiddle.id }</span>
                                    </div>
                                    <div class="ml10">试题内容：
														<span class="c_777 ">
														<textarea class="textarea ml10" style="width: 100%; ">${divpaperMiddle.qstContent}</textarea>
														</span>
                                    </div>
                                    <br/>

                                    <div class="" style="margin: 0 0 10px 30px">
                                        <c:if test="${divpaperMiddle.qstType!=6 }">
                                            <c:forEach items="${divpaperMiddle.options}" var="opt">
                                                ${opt.optOrder },${opt.optContent }<br/>
                                            </c:forEach>
                                        </c:if>
                                    </div>
                                    用户答案：
                                        <c:if test="${empty divpaperMiddle.userAnswer}">
                                            用户没有选择答案
                                        </c:if>
                                    <c:if test="${!empty divpaperMiddle.userAnswer}">
                                        ${divpaperMiddle.userAnswer}
                                    </c:if>
                                        &nbsp;正确答案：${divpaperMiddle.isAsr}
                                    <div class="" style="margin: 0 0 10px 30px">
                                        解析：<br />
                                        <textarea class="textarea" style="width: 100%; ">${divpaperMiddle.qstAnalyze }</textarea>
                                    </div>
                                </li>
