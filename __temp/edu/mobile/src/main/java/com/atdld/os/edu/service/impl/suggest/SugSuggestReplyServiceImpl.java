    ...
    public String addSugSuggestReply(SugSuggestReply sugSuggestReply) {
        ...
        if (userExpandService.checkLimitOpt(MemConstans.SUGGEST_REPLY_LIMIT, sugSuggestReply.getCusId())) {
            ...
            if (falgNum.intValue() == 1) {// 添加SugSuggestReply 成功 建议的回复数加一
                ...
                integralMap.put("keyWord",IntegralKeyword.answer_problem.toString() );
                integralMap.put("userId", sugSuggestReply.getCusId()+"" );
                integralMap.put("other",sugSuggestReply.getSuggestId()+"" );
                ...
                try {
                	...
                	//发送系统通知
                	webHessianService.mobileAddSystemMessageByCusId(userExpandDto.getShowname()+"回复"+"<a href='"+CommonConstants.contextPath+"/uc/question/info/"+sugSuggestReply.getSuggestId()+"'>"+title+"</a>", sugSuggest.getCusId());
				} catch (Exception e) {
					e.printStackTrace();
				}
    ...
    public Long updateSugSuggestReplyBySugSuggestReplyIdForIsBest(Long sugSuggestReplyId, Long sugSuggestId, int isBest, Long cusId) throws Exception {
        ...
        if (sugSuggest.getCusId().longValue() == cusId.longValue()) {// 验证要推荐为正确答案的建议是否是当前登陆用户创建的
            ...
            if (flagnum.intValue() > 0 && isBest == SuggestConstans.SUGSUGGEST_RECOMMEND_ISBEST_YES) {// 当更新的条数大于0时则更新建议为不可回复状态
                ...
                //采纳最佳答案添加积分
    			Map<String,String> integralMap=new HashMap<String, String>();
                integralMap.put("keyWord",IntegralKeyword.problem_accrept.toString() );
                integralMap.put("userId", sugSuggestReply.getCusId()+"" );
                integralMap.put("other",sugSuggestId+"" );
                integralMap.put("fromUserId",cusId+"" );
                integralMap.put("otherScorel","" );
                webHessianService.addUserIntegral(integralMap);
            }
