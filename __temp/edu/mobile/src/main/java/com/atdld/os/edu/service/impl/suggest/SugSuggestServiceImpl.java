...
public class SugSuggestServiceImpl implements SugSuggestService {
    ...
    public String addSugSuggest(SugSuggest sugSuggest) {
        ...
        if (userExpandService.checkLimitOpt(MemConstans.SUGGEST_LIMIT, sugSuggest.getCusId())) {
          ...
          //更新课程的笔记数量
            Map<String,String> peofileMap=new HashMap<String, String>();
            peofileMap.put("type", CourseProfileType.questiongcount.toString());//类型
            peofileMap.put("courseId",sugSuggest.getCourseId()+"");//课程id
            peofileMap.put("count", 1L+"");//1
            peofileMap.put("operation",CourseProfile.ADD);//操作
            webHessianService.updateCourseProfile(peofileMap);
    ...
    public String deleteSugSuggestById(Long id) throws Exception {
        ...
        if (falgNum > 0) {
           ...
           // 删除建议动态
			...
			//如果是课程问答 课程问答数减一
            if(sugSuggest.getCourseId()!=0){
                Map<String,String> peofileMap=new HashMap<String, String>();
                peofileMap.put("type", CourseProfileType.questiongcount.toString());//类型
                peofileMap.put("courseId",sugSuggest.getCourseId()+"");//课程id
                peofileMap.put("count", 1L+"");//1
                peofileMap.put("operation",CourseProfile.SUBTRACTION);//操作
                webHessianService.updateCourseProfile(peofileMap);
            }
