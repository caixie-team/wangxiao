...
import co.bluepx.edu.core.model.Deleted;
import co.bluepx.edu.core.util.ClassUtils;
import co.bluepx.edu.core.util.StringUtils;
import co.bluepx.edu.core.util.VelocityTemplateUtils;
...
//        System.out.println(StringUtils.camel2underscore(tableName) + "--------");
    ...
    private String insertSql(String tableName, List<String> columns) {
        ...
        while (i < columns.size()) {
            ...
            insertSql.append("<if test=\"").append(column).append(" != null\" >");
            insertSql.append(column).append(",");
            insertSql.append("</if>");
            valueSql.append("<if test=\"").append(column).append(" != null\" >");
            valueSql.append("#{").append(column).append("}").append(",");
            valueSql.append("</if>");
            i++;
        }
    ...
    private StringBuilder updateFields(String tableName, List<String> columns,
        ...
        for (String column : columns) {
            ...
            // if(useAlias) column = ;
            updateSql.append(" <if test=\"").append(columnAlias)
                    .append("!= null\"> ");
            updateSql.append(column).append("=#{").append(columnAlias)
                    .append("},");
            updateSql.append(" </if> ");
        }
