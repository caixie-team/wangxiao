...
function order() {
    ...
    if(buyCount==0){
        dialogFun('提示信息',"购物车内没有课程，请先选购课程！",0);
        return;
    }
        ...
        success: function(result) {
            if(result.success){
                if(isNotNull(result.entity.order)){
                    ...
                    dialogFun('友情提示',"参数错误",0);
                }else if(result.entity.msg=='amount'){
                    dialogFun('友情提示',"购物车金额错误",0);
                }else if(result.entity.msg=='courselosedate'){
                	dialogFun('友情提示',"购物车中有已经过期的课程，已经删除，请重新确认",5,'javascript:window.location.reload();');
                }
            }else{
				dialogFun('友情提示','下单异常，请稍后再试!',0);
            }
 		 ...
 		 $.ajax({
		   ...
		   success: function(result){
			   ...
			   if(result.message!='true'||obj==null){
				 initPrice();
				   dialogFun('友情提示',result.message,0);
                 return false;
			   }else{
				   $("#tjcode").hide();
				   $("#initcode").show();
