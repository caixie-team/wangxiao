...
function _create(expr, options) {
	...
	if (_undef(options.loadStyleMode, K.options.loadStyleMode)) {
		var themeType = _undef(options.themeType, K.options.themeType);
			_loadStyle('/kindeditor/themes/default/default.css');
	}
...
KindEditor.plugin('image', function(K) {
	var self = this, name = 'image',
		...
		imgPath = "/kindeditor/plugins" + '/image/images/',
		extraParams = K.undef(self.extraFileUploadParams, {}),
		filePostName = K.undef(self.filePostName, 'imgFile'),
...
/**
 *  K.uploadbutton kindeditor的上传图片封装
 * @param btnId 上传按钮id
 * @param urlId 图片展示id
 * @param valSet 图片地址存储元素id
 * @param type	上传图片类型，用于图片服务器存储位置
 */
function initSimpleImageUpload(btnId,urlId,valSet,type){
	KindEditor.ready(function(K) {
		var uploadbutton = K.uploadbutton({
			button : K('#'+btnId+'')[0],
			fieldName : "fileupload",
			url : imageUrl+'/gok4?base=eduplat&param='+type,
			afterUpload : function(data) {
				if (data.error === 0) {
					var url = K.formatUrl(data.url, 'absolute');//absolute,domain
					K('#'+urlId+'').attr("src",staticUrl+data.url);
					$("#"+urlId).show();
					$('#'+valSet+'').val(url);
				} else {
					alert(data.message);
...
//添加博文页面编辑器
function initKindEditor_addblog(id, width, height,type) {
	EditorObject = KindEditor.create('textarea[id=' + id + ']', {
		...
		minWidth : '10px',
		minHeight : '10px',
		uploadJson : imageUrl+'/imgk4?param='+type,// 图片上传路径
		afterBlur : function() {
			this.sync();
		},
