...
$(function() {
	...
	/*if(isNotEmpty(currentKpointId)){
	 ...
	 /*  alert(currentCourseId);
	   alert(currentKpointId);*/
});
		...
		//当前展示的树的id
 		if(prevoUrlObjs&&prevoUrlObjs!=null&&prevoUrlObjs.name!=''){
 			var title = prevoUrlObjs.name;
	   		$("#upLast").html('<a onclick="clickPlay(null,\'\',null,null,'+prevoUrlObjs.id+')" href="javascript:void(0)">上一节课：'+title+'</a>');
 		}else{
 		...
 		if(nextvoUrlObjs&&nextvoUrlObjs!=null&&nextvoUrlObjs.name!=''){
 			var title = nextvoUrlObjs.name;
	   		$("#downLast").html('<a onclick="clickPlay(null,\'\',null,null,'+nextvoUrlObjs.id+')" href="javascript:void(0)">下一节课：'+title+'</a>');
 		}else{
...
function addcurrentKpointId(kpointid,videourl,name){
	 currentKpointId = kpointid;
	 player(currentCourseId,currentKpointId,videourl,name);//初始化课程并播放
}

function player(courseId,kpointId,videourl,name){
	//判断是否登录
	if(!isLogin()){
		window.location.href=baselocation+"/login";
		return;
	}
	if(checkSellWayById(courseId,kpointId)){
		if(name!=null &&name!=''){
	 		var initTitle = name;
	 		if(initTitle.indexOf('<b')>-1){
	 			initTitle = initTitle.substring(0,initTitle.indexOf('<b'));
	...
	getPlayerHtml(courseId,kpointId);
	//记录播放次数
			...
			success: function (result){
				if(result.success){
					...
					for(var i=0;i<res.length;i++){
						res[i].title=res[i].name.split("\<")[0];
						if(res[i].name.length>20){
							res[i].name=res[i].name.substring(0,20);
						}
					}
				}
			}
