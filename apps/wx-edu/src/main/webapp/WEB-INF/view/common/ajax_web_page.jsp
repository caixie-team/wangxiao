...
<c:if test="${page != null && page.totalResultSize>0}">
	...
	<div class="paging">
		<c:choose>
		   ...
		   <c:otherwise>
			  <a class="page-side active" href="javascript:goPageAjax(1);" title="首页">首</a>
		   </c:otherwise>
		</c:choose>
		<c:choose>
		   <c:when test="${page.first}">
		   ...
		   <c:otherwise>
			  <a id="backpage" class="active" href="javascript:goPageAjax(${page.currentPage-1 });" title="上一页"><em class="icon12 page-prev"></em></a>
		   </c:otherwise>
		</c:choose>
		<c:choose>
		   ...
		   <c:otherwise>
			 <a id="nextpage" class="active" href="javascript:goPageAjax(${page.currentPage+1 })" title="下一页"><em class="icon12 page-next"></em></a>
		   </c:otherwise>
		</c:choose>
		<c:choose>
		   <c:when test="${page.last}">
		   ...
		   <c:otherwise>
			 <a class="page-side active" href="javascript:goPageAjax(${page.totalPageSize })" title="末页">末</a>
		   </c:otherwise>
		</c:choose>
		<div class="clear"></div>
	</div>
...
<script type="text/javascript">	
	...
	function showPageNumber() {
		...
		for ( var i = 3; i > 0; i--) { //<a href="" title="">3</a>
			if (currentPage > i) {
				pageHtml = pageHtml + "<a href='javascript:goPageAjax("
