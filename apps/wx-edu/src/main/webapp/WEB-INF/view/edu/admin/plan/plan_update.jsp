...
<head>
	...
	<script type="text/javascript">
		$(function () {
			var status = ${planOld.status};
			$("#status").val(status);
			var isRepeat = ${planOld.isRepeat};
		...
		function showNewExam() {
			...
			window.open('${ctx}/admin/plan/getTaskExamList', 'newwindow', 'toolbar=no,scrollbars=yes,location=no,resizable=no,top=200,left=300,width=1250,height=600');
		}
		...
		function queryExam() {
			...
			if (id != null && id != "") {
				$.ajax({
					type: "POST",
					dataType: "json",
					url: "${ctx}/admin/plan/queryByExamIds",
					data: {
						"id": id
					},
					async: false,
		...
		function getPhaseListAjax(planId) {
			$.ajax({
				url: "${ctx}/admin/phase/getPhaseListAjax/" + planId,
				data: {},
				dataType: "text",
				type: "post",
				async: "true",
		...
		function AddStudyPhase() {
			...
			if (isNotEmpty(planId)) {
				$.ajax({
					url: "${ctx}/admin/phase/addPhase/" + planId,
					data: {},
					dataType: "json",
					type: "post",
					async: "true",
		...
		function delStudyPhaseRank() {
			...
			var phaseId = $("#phaseId").val();
			$.ajax({
				url: "${ctx}/admin/phase/delPhaseRank/" + planId,
				data: {
					"theId": phaseId
				},
				dataType: "json",
...
<div class="mt20">
	<div class="commonWrap">
		<form action="/admin/plan/updateTask" method="post" id="updateTask">
			<input type="hidden" id="planType" name="plan.type" value="${planOld.type}" />
			<input type="hidden" id="emIds" name="plan.examIds" value="${planOld.examIds}" />
			<input type="hidden" id="courseIdHidden" name="courseIds" />
			<input type="hidden" id="emNames" name="plan.examNames" />
			<input type="hidden" id="groupIds" name="plan.groupIds" />

			<input type="hidden" id="planId" name="plan.id" value="${planOld.id}" />
			<input type="hidden" id="planSubmit" name="plan.submit" value="${planOld.submit}" />
			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0" class="commonTab01">
				...
				<tr>
					...
					<td>
						<input name="plan.name" id="name" value="${planOld.name}">
				...
				<tr>
					...
					<td>
						<input name="plan.beginTime" id="beginTime" readonly="readonly" value="<fmt:formatDate value="${planOld.beginTime }" pattern="yyyy-MM-dd HH:mm:ss" />">
				...
				<tr>
					...
					<td>
						<input name="plan.endTime" id="endTime" readonly="readonly" value="<fmt:formatDate value="${planOld.endTime }" pattern="yyyy-MM-dd HH:mm:ss" />">
			...
			<table width="100%" cellspacing="0" cellpadding="0" border="0" class="commonTab01">
				...
				<tr>
					<td width="20%" align="center"><font color="red">*</font>&nbsp;当前计划概要</td>
					<td>
						<select name="plan.status" id="status">
				...
				<tr>
					...
					<td>
						<select name="plan.isRepeat" id="isRepeat">
