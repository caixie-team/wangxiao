...
function submitAddArrange(status){
	...
	if(isEmpty($("#examName").val())){
		dialogFun("安排考试","请填写考试名称",0);
		return;
	}
	if(isEmpty($("#groupIds").val())){
		dialogFun("安排考试","请选择所属岗位",0);
		return;
	}
	if(isEmpty($("#emIds").val())){
		dialogFun("安排考试","请添加试卷",0);
		return;
	}
	var beginTimeStr=$("#beginTime").val();
	var endTimeStr=$("#endTime").val();
	if(isEmpty(beginTimeStr)){
		dialogFun("安排考试","请选择考试开始时间",0);
		return;
	}
	if(isEmpty(endTimeStr)){
		dialogFun("安排考试","请选择考试结束时间",0);
		return;
	}
	...
	if(beginTime<current){
		dialogFun("安排考试","考试开始时间应大于当前时间",0);
		return;
	}
	if(endTime<current){
		dialogFun("安排考试","考试结束时间应大于当前时间",0);
		return;
	}
	if(beginTime>=endTime){
		dialogFun("安排考试","考试结束时间应大于考试开始时间",0);
		return;
	}
	$("#openarrange").submit();
}
...
<body>
	<div class="am-cf">
		<div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">考试管理</strong> / <small>新建岗位考试</small></div>
	</div>
	<hr/>
	<div class="mt20">
		<div class="am-tabs">
			...
			<div class="am-tabs-bd">
				<div id="tab1" class="am-tab-panel am-fade am-active am-in">
					<form id="openarrange" action="${ctx}/admin/arrange/addArrange" class="am-form am-form-inline"  data-am-validator>
						...
						<input id="type" type="hidden" name="arrange.type" value="1" />
						...
						<div class="am-g am-margin-top">
							...
							<div class="am-u-sm-8 am-u-md-6">
								<input required name="arrange.name" id="examName" type="text" class="am-input-sm">
							</div>
						...
						<div class="am-g am-margin-top">
							...
							<div class="am-u-sm-8 am-u-md-6">
								<select disabled class="am-input-sm">
									<option value="1" selected="selected">岗位</option>
								</select>
							</div>
							<div class="am-hide-sm-only am-u-md-4 am-text-danger">*必填</div>
						</div>
						...
						<div class="am-g am-margin-top">
							...
							<div class="am-u-sm-8 am-u-md-6">
								<div class="am-form-group am-form-icon">
									...
									<input type="text" name="arrange.beginTime" id="beginTime" readonly placeholder="开始时间" class="form-datetime-lang am-form-field am-input-sm">
								</div>
							</div>
							<div class="am-hide-sm-only am-u-md-4 am-text-danger">*必填</div>
						</div>
						<div class="am-g am-margin-top">
							...
							<div class="am-u-sm-8 am-u-md-6">
								<div class="am-form-group am-form-icon">
									<i class="am-icon-calendar"></i>
									<input type="text" name="arrange.endTime" id="endTime" readonly placeholder="结束时间" class="form-datetime-lang am-form-field am-input-sm">
								</div>
							</div>
							<div class="am-hide-sm-only am-u-md-4 am-text-danger">*必填</div>
						</div>
						<div class="am-g am-margin-top">
							...
							<div class="am-u-sm-8 am-u-md-6">
								...
								<button class="am-btn am-btn-danger" type="button" onclick="submitAddArrange('RELEASE')">发布</button>
							</div>
