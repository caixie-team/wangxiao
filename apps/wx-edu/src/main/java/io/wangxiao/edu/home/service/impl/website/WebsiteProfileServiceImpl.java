...
public class WebsiteProfileServiceImpl implements WebsiteProfileService {
    ...
    public void updateWebsiteProfile(WebsiteProfile websiteProfile) throws Exception {
        ...
        cacheKit.remove(MemConstans.WEBSITE_PROFILE + websiteProfile.getType());
        ehcache.remove(MemConstans.WEBSITE_PROFILE + websiteProfile.getType());
    }
    ...
    public Map<String, Object> getWebsiteProfileList() throws Exception {
        ...
        List<String> websitesStr = (List<String>) cacheKit.get(MemConstans.WEBSITE_PROFILE);
        if (ObjectUtils.isNull(websitesStr) || websitesStr.size() == 0) {
            ...
            cacheKit.set(MemConstans.WEBSITE_PROFILE, websitesStr, MemConstans.WEBSITE_PROFILE_TIME);
        }
    ...
    public Map<String, Object> getWebsiteProfileByType(String type) {
        ...
        String websiteProfileStr;
        //Ehcache存储获取
        websiteProfileStr = (String) ehcache.get(MemConstans.WEBSITE_PROFILE + type);
        if (StringUtils.isEmpty(websiteProfileStr)) {
            //redis服务器获取
            websiteProfileStr = (String) cacheKit.get(MemConstans.WEBSITE_PROFILE + type);
            if (StringUtils.isNotEmpty(websiteProfileStr) && !"null".endsWith(websiteProfileStr)) {
                ehcache.put(MemConstans.WEBSITE_PROFILE + type, websiteProfileStr);
            }
        }
        if (ObjectUtils.isNull(websiteProfileStr) || "null".equals(websiteProfileStr)) {//memcache为空查询数据库
            ...
            cacheKit.set(MemConstans.WEBSITE_PROFILE + type, websiteProfileStr, MemConstans.WEBSITE_PROFILE_TIME);//设置key 时间一天
        }
