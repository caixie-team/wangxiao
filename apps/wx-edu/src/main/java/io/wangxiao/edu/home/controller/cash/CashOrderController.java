...
public class CashOrderController extends EduBaseController {
    ...
    private static final String cash_pay = getViewPath("/ucenter/cash_pay");
    private static final String cash_cart = getViewPath("/cash/cashCart");
    private static final String order_list = getViewPath("/cash/u_recharge_order");
    private static final String cash_repay = getViewPath("/cash/cash_repay");
    // 微信支付二维码
    private static final String qcCode = getViewPath("/shopcart/QRCode");
     ...
     * @throws Exception
     */
    @RequestMapping("/uc/cash/dopay")
    public String cashDoPay(HttpServletRequest request) {
        try {
            UserAccount userAccount = userAccountService.getUserAccountByUserId(getLoginUserId(request));
            request.setAttribute("balance", userAccount.getBalance().doubleValue());
        } catch (Exception e) {
            logger.error("CashOrderController.cashDoPay error", e);
    ...
    public String payCashOrder(HttpServletRequest request) {
        try {
            String payCash = request.getParameter("payCash");
            request.setAttribute("payCash", payCash);
        } catch (Exception e) {
            logger.error("CashOrderController.payCashOrder error", e);
            return this.setExceptionRequest(request, e);
        }
        return cash_cart;
    }
        ...
        try {
            ...
            if (ObjectUtils.isNotNull(cashOrder)) {
                if (TrxOrderStatus.SUCCESS.toString().equals(cashOrder.getTrxStatus())) {
                    redirectAttributes.addAttribute(OrderConstans.RESMSG, "订单已支付成功");
                    return "redirect:/front/success";
                }
                if (!TrxOrderStatus.INIT.toString().equals(cashOrder.getTrxStatus())) {
                    redirectAttributes.addAttribute(OrderConstans.RESMSG, "订单信息异常，请稍后再试");
                    return "redirect:/front/success";
                }
                // 走银行流程
                Map<String, String> sourceMap = new HashMap<String, String>();
                ...
                if (payType.equals(PayType.ALIPAY.toString())) {
                    ...
                    // 尝试支付余额不足,进行微信扫码支付
                    String wxPayUrl = weixinPayService.getWxpayUrl(cashOrder.getRequestId(), "cash");
                    ...
                    request.setAttribute("type", "cash");
                    return qcCode;
                }
            } else {
                redirectAttributes.addAttribute(OrderConstans.RESMSG, "订单信息异常，请稍后再试");
