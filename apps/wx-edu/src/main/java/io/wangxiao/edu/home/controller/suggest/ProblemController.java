...
/*    @RequestMapping(value = "/ques/my")
    ...
    public ModelAndView suglist(HttpServletRequest request, @ModelAttribute("page") PageEntity page) {
        ...
        try {
            ...
            } else if ("solved".equals(flag)) {
                QuerySugSuggest querySugSuggest = new QuerySugSuggest();
                List<SugSuggest> SugSuggestList = sugSuggestService.querySuggestRecommend(querySugSuggest, page);
                modelAndView.addObject("SugSuggestList", SugSuggestList);// 把值得出的分页的值
            } else if ("mysug".equals(flag)) {
                List<SugSuggest> SugSuggestList = sugSuggestService.querySuggestByCusIdAndStatus(getLoginUserId(request), SuggestConstans.SUGSUGGEST_STATUS_CANREPLY, page);// 通过cusId查询我寻求的建议
                modelAndView.addObject("SugSuggestList", SugSuggestList);// 把值得出的分页的值
            }
            //我回答的问题
            else if ("mysughd".equals(flag)) {
                List<SugSuggest> sugSuggestList = sugSuggestService.querySuggestByReplyCusId(getLoginUserId(request), page);// 查询我回答的建议
                modelAndView.addObject("SugSuggestList", sugSuggestList);// 把值得出的分页的值
            }
            //我推荐的问题
            else if ("myrecommend".equals(flag)) {
    ...
    public String toSugSuggestInfo(@PathVariable Long sugSuggestId, HttpServletRequest request,
        ...
        try {
            ...
            List<SugSuggestReply> sugSuggestReplyList = sugSuggestReplyService.querySugSuggestReplyListBySuggestId(sugSuggestId, SuggestConstans.SUGSUGGEST_RECOMMEND_ISBEST_YES, page);// 该建议下的是最佳答案的回复
    ...
    public ModelAndView querySugSuggestReplyList(HttpServletRequest request, @ModelAttribute("page") PageEntity page
        ...
        try {
            ...
            List<SugSuggestReply> sugSuggestReplyList = sugSuggestReplyService.querySugSuggestReplyListBySuggestId(sugSugestId, SuggestConstans.SUGSUGGEST_RECOMMEND_ISBEST_NO, page);// 该建议下的不是最佳答案的回复
