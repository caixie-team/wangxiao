package io.wangxiao.edu.mobile.order;
...
import io.wangxiao.edu.mobile.order.util.*;
...
public class MobileTrxorderController extends MobileBaseController {
    ...
    public String courseOrderList(HttpServletRequest request, @ModelAttribute("page") PageEntity page,
                                  QueryTrxorder queryTrxorder) {
        try {

            page.setPageSize(6);
            page.setCurrentPage(1);// 非ajax从第一页开始
    ...
    public String courseOrderListAjax(HttpServletRequest request, @ModelAttribute("page") PageEntity page,
                                      QueryTrxorder queryTrxorder) {
        try {
    ...
    public String liveOrderList(HttpServletRequest request, @ModelAttribute("page") PageEntity page,
                                QueryTrxorder queryTrxorder) {
        try {
    ...
    public String liveOrderListAjax(HttpServletRequest request, @ModelAttribute("page") PageEntity page,
                                    QueryTrxorder queryTrxorder) {
        try {
    ...
    @ResponseBody
    public Map<String, Object> createMobileTrxorder(HttpServletRequest request, HttpServletResponse response)
            throws Exception {
        ...
        try {
            ...
            if (ObjectUtils.isNotNull(trxorder)) {
                ...
                if (res.get(OrderConstans.RESCODE).equals(OrderConstans.SUCCESS)) {
                    ...
                    return "redirect:/to/alipay?requestId=" + res.get("requestId") + "&bankAmount="
                            + res.get("bankAmount");
                } else {// 优惠券错误信息
                    redirectAttributes.addAttribute(OrderConstans.RESMSG, res.get(OrderConstans.RESMSG));
    ...
    public void gotoalipay(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String notify_url = CommonConstants.contextPath + "/mobile/order/alipay/back/" + getLoginUserId(request)
                    + "/" + 1;// 服务器异步通知页面路径
            String return_url = CommonConstants.contextPath + "/mobile/order/alipay/back/" + getLoginUserId(request)
                    + "/" + 2;// 页面跳转同步通知页面路径
            ...
            // 建立请求
            String sHtmlText = AlipaySubmit.buildRequest(sParaTemp, websitemap.get("privatekey"), "get", "确认");
    ...
    public String alipayBack(HttpServletRequest request, @PathVariable String userId, @PathVariable("rtype") Long rtype,
        ...
        try {
            ...
            if (verify_result) {// 验证成功
                ...
                sourceMap.put("userId", getLoginUserId(request) + "");
    ...
    @ResponseBody
    public Map<String, Object> gotoWeixinbank(HttpServletRequest request, HttpServletResponse response,
                                              @RequestParam(value = "orderId", required = true) Long orderId) {
        Map<String, Object> json = null;
        try {
            // 查询订单
            ...
            if (ObjectUtils.isNotNull(trxorder)) {
                ...
                if (res.get(OrderConstans.RESCODE).equals(OrderConstans.SUCCESS)) {
                    ...
                    if (openId == null) {
                        json = this.getJsonMap(false, "订单异常，请稍后再试", null);
                        return json;
                    }
                    // 微信支付生成支付url
                    String prepay_id = bizpaygetpackage(request, trxorder.getId(), openId);
                    if (prepay_id.equals("")) {
                        json = this.getJsonMap(false, "订单异常，请稍后再试", null);
                        return json;
                    }
                    Map<String, String> map = getWxpayUrl(request, prepay_id);
                    json = this.getJsonMap(true, trxorder.getUserId().toString(), map);

                } else {// 优惠券错误信息
                    json = this.getJsonMap(false, res.get(OrderConstans.RESMSG), null);
                    return json;
     ...
     * @return
     */
    public String bizpaygetpackage(HttpServletRequest request, Long id, String openId) {
        String prepay_id = "";
        try {
            getWxpayInfo();
            // 请求参数包
            SortedMap<String, String> packageParams = createPackage(id, openId);
            // sign签名
            ...
            if (returnCode.equalsIgnoreCase("FAIL")) {
                ...
                if (result_code.equalsIgnoreCase("FAIL")) {
                    logger.info("result_code-----fail:err_code" + resultMap.get("err_code") + "/n" + "err_code_des"
     ...
     * @return
     */
    public SortedMap<String, String> createPackage(Long id, String openId) {
        // 订单信息
    ...
    public void wxpayrtn(HttpServletRequest request, HttpServletResponse response,
    ...
    public void sendXmlMessage(HttpServletRequest request, HttpServletResponse response, String content)
            throws IOException {
        try {
