...
public class Tag extends WeiboResponse implements java.io.Serializable {
	...
	public static List<weibo4j.model.Tag> constructTags(Response res) throws WeiboException {
		try {
			JSONArray list = res.asJSONArray();
			int size = list.length();
			List<weibo4j.model.Tag> tags = new ArrayList<weibo4j.model.Tag>(size);
			for (int i = 0; i < size; i++) {
				tags.add(new weibo4j.model.Tag(list.getJSONObject(i)));
			}
			return tags;
	...
	public static TagWapper constructTagWapper(Response res){
		try {
			...
			List<weibo4j.model.Tag> tagList = new ArrayList<weibo4j.model.Tag>();
			for(int i=0;i<tags.getJSONObject(0).getJSONArray("tags").length();i++){
				tagList.add(new weibo4j.model.Tag(tags.getJSONObject(0).getJSONArray("tags").getJSONObject(i)));
			}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		weibo4j.model.Tag other = (weibo4j.model.Tag) obj;
		if (id == null) {
			if (other.id != null)
				return false;
