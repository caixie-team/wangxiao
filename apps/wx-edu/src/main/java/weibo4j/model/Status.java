...
public class Status extends WeiboResponse implements java.io.Serializable {
	...
	private weibo4j.model.Status retweetedStatus = null;               //转发的博文，内容为status，如果不是转发，则没有此字段
	...
	private void constructJson(JSONObject json) throws WeiboException {
		try {
			...
			if(!json.isNull("retweeted_status")){
				retweetedStatus= new weibo4j.model.Status(json.getJSONObject("retweeted_status"));
			}
	...
	public weibo4j.model.Status getRetweetedStatus() {
		return retweetedStatus;
	}
	public void setRetweetedStatus(weibo4j.model.Status retweetedStatus) {
	...
	public static StatusWapper constructWapperStatus(Response res) throws WeiboException {
		...
		try {
			...
			List<weibo4j.model.Status> status = new ArrayList<weibo4j.model.Status>(size);
			for (int i = 0; i < size; i++) {
				status.add(new weibo4j.model.Status(statuses.getJSONObject(i)));
			}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		weibo4j.model.Status other = (weibo4j.model.Status) obj;
		if (id == null) {
			if (other.id != null)
				return false;
