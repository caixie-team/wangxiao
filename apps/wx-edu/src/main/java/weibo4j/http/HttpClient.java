...
public class HttpClient implements java.io.Serializable {
	...
	static Logger log = LoggerFactory.getLogger(weibo4j.http.HttpClient.class.getName());
	...
	public Response get(String url, PostParameter[] params)
		...
		if (null != params && params.length > 0) {
			String encodedParams = weibo4j.http.HttpClient.encodeParameters(params);
			if (-1 == url.indexOf("?")) {
				url += "?" + encodedParams;
			} else {
				url += "&" + encodedParams;
		...
		if (null != paging) {
			...
			if (null != params) {
				...
				if (0 != arrayPagingParams.length) {
					String encodedParams = weibo4j.http.HttpClient
